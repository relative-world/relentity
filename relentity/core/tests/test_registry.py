...
async def test_register_entity(registry):
    ...
    assert entity.id in registry.entities
    assert registry.entities[entity.id] is entity


@pytest.mark.asyncio
async def test_entities_with_components(registry):
    """Test querying entities with specific components."""
    entity = Entity[Position(x=10, y=10), Velocity(vx=5, vy=5)](registry)

    entities_with_pos = [await e.resolve() async for e in registry.entities_with_components(Position)]
    entities_with_pos_vel = [await e.resolve() async for e in registry.entities_with_components(Position, Velocity)]
...
async def test_remove_component_from_entity(registry):
    ...
    await registry.remove_component_from_entity(entity.id, Velocity)
